...
def main():
	...
	if not path.exists(wpath+"/Dockerfile"):
		...
		wpath = input(yellow+" Path to the place with your Dockerfile: "+normal)
		main()
	else:
		print (green+" Dockerfile was found."+normal)
		print (" Have you signed into the Registry you want to publish this image on")
		choice = input(" already (y/n): ")
	if (choice=="y" or choice=="Y"):
		registryok(999)
	else:		
		registry_url = input(green+" OK. No panic. Give me the registry URL (defaults to registry.gitlab.com)\n"+normal+" Registry URL: [or enter to default]: ")
		if(registry_url==""):
...
def registryok(value):
	...
	projecturl = input(" What's the Registry's image URL? [ex: registry.gitlab.com/user/project]\n URL: ")
	projectUrlOk()
